    ...
    <div className="h-[calc(100vh-3.5rem)] w-full p-6 lg:p-8 overflow-y-auto">
      <div className="h-full max-w-4xl mx-auto">
        ...
        <div className="bg-gteal rounded-xl shadow-lg p-8 mb-6">
          ...
          <p className="text-blue">Share recipes, tips, and healthy eating advice</p>
        </div>

        {/* Tabs */}
        <div className="bg-gteal rounded-xl shadow-lg mb-6">
            ...
            <button
              ...
              className={`flex-1 px-6 py-4 font-semibold transition ${
                activeTab === 'all'
                  ...
                  : 'text-blue hover:text-emerald-600'
              }`}
            >
              All Discussions
            </button>
        ...
        {/* New Message Section */}
        <div className="bg-gteal rounded-xl shadow-lg p-6 mb-6">
        ...
        <div className="space-y-4">
          {filteredMessages.map((message) => (
            <div
              key={message.id}
              className="bg-gteal rounded-xl shadow-lg p-6 hover:shadow-xl transition"
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-semibold text-blue">{message.author}</p>
                    <p className="text-sm text-gray-500">{message.timestamp}</p>
                  </div>
                </div>
              </div>
              
              <p className="text-blue mb-4 leading-relaxed">{message.text}</p>
              
              <div className="flex items-center space-x-6">
                <button
                  onClick={() => handleLike(message.id)}
                  className="flex items-center space-x-2 text-blue hover:text-emerald-600 transition"
                >
                ...
                <button className="flex items-center space-x-2 text-blue hover:text-emerald-600 transition">
        ...
        {/* Stats */}
        <div className="mt-8 bg-gteal rounded-xl shadow-lg p-6">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-3xl font-bold text-emerald-700">{messages.length}</p>
              <p className="text-blue text-sm">Discussions</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-emerald-700">
                {messages.reduce((sum, msg) => sum + msg.likes, 0)}
              </p>
              <p className="text-blue text-sm">Total Likes</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-emerald-700">
                {new Set(messages.map(m => m.author)).size}
              </p>
              <p className="text-blue text-sm">Active Members</p>
            </div>
          </div>
        </div>
      </div>
